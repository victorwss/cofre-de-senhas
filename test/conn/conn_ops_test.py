...
from ..fixtures import assert_dbf_ok, dbs_f, applier, applier_trans, applier_trans2, DbTestConfig
from ..fixtures import mariadb_db_f, sqlite_db_f, mysql_db_f
...
@applier_trans(dbs_f, assert_dbf_ok)
...
@applier_trans2(dbs_f, assert_dbf_ok)
...
def test_lastrowid_none(c: TransactedConnection, db: DbTestConfig) -> None:
    ...
    assert t is None
    assert c.lastrowid == 0 if db == sqlite_db_f else c.lastrowid is None  # Sqlite is buggy and gives zero instead of None!
