...
def test_instanciar() -> None:
    s: SegredoDAO = SegredoDAOImpl(db.raiz)
    assert s == SegredoDAO.instance()

@db.transacted
def test_criar_segredo() -> None:
    dao: SegredoDAOImpl = SegredoDAOImpl(db.raiz)
...
def test_criar_permissao_usuario_nao_existe() -> None:
    dao1: SegredoDAOImpl = SegredoDAOImpl(db.raiz)
...
def test_criar_categoria_segredo() -> None:
    ...
    dao2: CategoriaDAOImpl = CategoriaDAOImpl(db.raiz)
